"""
High-Performance 1C Contact Extraction via COMConnector (ValueTable Batch Unload).

Extracts:
  - Counterparty Name (Назва) & Full Legal Name (Повна Назва)
  - EDRPOU (Код по ЕГРПОУ / ЄДРПОУ) & INN (ІПН)
...
def extract_utp_full_contacts():
    ...
    logger.info("Unloaded ValueTable from 1C UTP: {} rows, {} columns!", total_rows, len(cols))
    ...
    logger.info("Consolidated {} unique counterparties with full contact details in 1C UTP!", len(df_clean))
