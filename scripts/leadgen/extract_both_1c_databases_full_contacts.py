"""
Fast Senior-Level Contact Extraction Pipeline for 1C UTP (IPT) and 1C Magazin.

Uses vectorized Pandas grouping on 1C ValueTables to extract:
  ...
  - Legal & Physical Addresses (Юридичні та Фактичні адреси)
"""
...
DATABASES = [
    {"name": "UTP_IPT", "path": BASE_DIR + r"\UTP", "user": "Админ", "pwd": "Cjpthwfybt!33", "color": "1F4E78"},
    {"name": "Magazin", "path": BASE_DIR + r"\Магазин", "user": "admin", "pwd": "Cjpthwfybt!33", "color": "2E75B6"},
]
...
def extract_db_contacts(db_info):
    ...
    logger.info("Connecting to 1C [{}]...", db_info['name'])

    connector = win32com.client.Dispatch("V83.COMConnector")
    v83 = connector.Connect(conn_str)
    logger.info("Connected to 1C [{}]!", db_info['name'])
    ...
    logger.info("Unloaded ValueTable from 1C [{}]: {} rows!", db_info['name'], total_rows)
    ...
    aggregated.rename(columns={
        "Код": "Код 1С",
        "Назва": f"Контрагент (1С {db_info['name']})",
        "ПовнаНазва": "Повна Назва",
        "КонтактнаОсоба": "Контактна Особа",
    }, inplace=True)

    logger.info("Consolidated {} counterparties for 1C [{}]!", len(aggregated), db_info['name'])

    out_csv = OUT_DIR / f"1c_contacts_{db_info['name']}.csv"
    aggregated.to_csv(out_csv, index=False, encoding="utf-8-sig")

    out_excel = OUT_DIR / f"1c_contacts_{db_info['name']}.xlsx"
    with pd.ExcelWriter(out_excel, engine="openpyxl") as writer:
        aggregated.to_excel(writer, index=False, sheet_name=f"Контакти 1С {db_info['name']}")
        format_excel_sheet(writer.sheets[f"Контакти 1С {db_info['name']}"], aggregated, header_color=db_info["color"])

    logger.info("Saved 1C [{}] Contacts -> {}", db_info['name'], out_excel)
    return aggregated


def main():
    logger.info("Executing Vectorized 1C Contact Extraction for UTP and Magazin...")
    for db in DATABASES:
        extract_db_contacts(db)
    logger.info("All 1C Contacts Extracted Successfully!")
