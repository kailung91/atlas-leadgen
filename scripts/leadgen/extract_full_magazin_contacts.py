"""
Extract Full Contact Information Register for 1C Magazin Database.
User: 'admin', Password: 'Cjpthwfybt!33'
"""
...
def extract_magazin_full_contacts():
    ...
    logger.info("Unloaded ValueTable from 1C Magazin: {} rows, {} columns!", total_rows, len(cols))

    rows = []
    for idx in range(total_rows):
        row_obj = vt.Получить(idx)
        row_dict = {col: unwrap(getattr(row_obj, col)) for col in cols}
        rows.append(row_dict)

    del v83, connector
    df_raw = pd.DataFrame(rows)
    logger.info("Converted ValueTable to DataFrame: {} rows!", len(df_raw))
    ...
    logger.info("Consolidated {} unique counterparties with contact details in 1C Magazin!", len(df_clean))
    ...
    with pd.ExcelWriter(out_excel, engine="openpyxl") as writer:
        df_clean.to_excel(writer, index=False, sheet_name="Контакти 1С Магазин")
        format_excel_sheet(writer.sheets["Контакти 1С Магазин"], df_clean, header_color="2E75B6")
